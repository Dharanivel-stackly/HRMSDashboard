...
import { useManagerTasks } from '@/features/hrms/onboarding/hooks/useDocumentUpload';
...
export default function ManagerTasks() {
  const { data: tasks, isLoading, isError, refetch } = useManagerTasks();
  ...
  return (
    <PageContainer className="bg-gradient-to-b from-[#f0f4ff] to-white min-h-screen">
      <PageHeader
        title="Manager Tasks"
        description="Track manager tasks for new employees"
      />
        ...
        <Table>
          ...
          <TableBody>
            {list.length === 0 ? (
              ...
              list.map((task) => (
                <TableRow key={task.id} className="hover:bg-[#f0f4ff]/40 transition-colors">
